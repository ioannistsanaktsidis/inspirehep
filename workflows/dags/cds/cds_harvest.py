...
def cds_harvest_dag():
    ...
    def process_cds_response(cds_record):
        ...
        def process_record(cds_record):
            ...
            try:
                record = inspire_http_record_management_hook.get_record(
                    pid_type="literature", control_number=record_id
                )
        ...
        def build_record(payload):
            ...
            revision = payload["original_record"].get("revision_id", 0)

            builder = LiteratureBuilder(record=payload["original_record"]["metadata"])
        ...
        def update_inspire_record(payload):
            ...
            logger.info(
                f"Updating record {payload['updated_record']}"
                f"with revision ID {revision_id}"
            )
